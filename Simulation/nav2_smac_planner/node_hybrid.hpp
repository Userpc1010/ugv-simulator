...
typedef std::pair<float, float> TrigValues;

typedef std::pair<float, uint32_t> ObstacleHeuristicElement;
...
public:
   ...
   */
  explicit NodeHybrid(const uint32_t index);
  ...
  {
    return static_cast<uint32_t>(angle) + static_cast<uint32_t>(x) *
           static_cast<uint32_t>(angle_quantization) +
  ...
  static std::shared_ptr<Costmap2D_Master> costmap_master;
