...
#include "analytic_expansion.hpp"
...
template<typename NodeT>
int AnalyticExpansion<NodeT>::countDirectionChanges(const ompl::base::ReedsSheppStateSpace::PathType & path)
{
  const float * lengths = path.length_;
  int changes = 0;
  int last_dir = 0;
  for (int i = 0; i < 5; ++i) {
    if (lengths[i] == 0.0f) {
    ...
    int currentDirection = (lengths[i] > 0.0f) ? 1 : -1;
  ...
  const ompl::base::StateSpacePtr & state_space)
{
  ...
  if (!failure) {
    ...
    if (max_cost_it != node_costs.end() && *max_cost_it > max_cost) {
      ...
      if (failure) {
        if (d < 2.0f * M_PIf32 * goal->motion_table.min_turning_radius &&
