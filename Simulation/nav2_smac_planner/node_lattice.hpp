...
struct LatticeMotionTable
{
  ...
  /**
   * @brief A destructor for nav2_smac_planner::LatticeMotionTable
   */
  ~LatticeMotionTable();

  // получения структур с примитивами
  struct PrimitiveBuffers {
      MotionPrimitive* forward;        // Указатель на начало массива прямых примитивов
      size_t forward_count;
      MotionPrimitive* reverse;        // Указатель на начало массива обратных примитивов
      size_t reverse_count;

      // Проверка валидности
      bool isValid() const { return forward != nullptr; }
  };
   ...
   */
  PrimitiveBuffers getPrimitiveBuffers(const NodeLattice* node) const;
   ...
   */
  unsigned int getClosestAngularBin(const float & theta);
   ...
   * @return bin index of closest angle to request
   */
  float getAngle(const float & theta);

  // Функция для освобождения памяти
  void cleanupMotionPrimitives();

  void cleanupFastAccessTables();

  void createFastAccessTables();
...
//  std::vector<std::vector<MotionPrimitive>> motion_primitives;
  ...
  MotionPrimitive* motion_primitives_flat = nullptr;
  size_t motion_primitives_flat_size = 0;
  std::vector<size_t> motion_offsets;
  std::vector<size_t> motion_counts;

  struct AnglePrimitives {
      MotionPrimitive* forward_primitives;   // Указатель на начало массива прямых примитивов
      size_t forward_count = 0;
      MotionPrimitive* reverse_primitives;   // Указатель на начало массива обратных примитивов
      size_t reverse_count = 0;
  };

 AnglePrimitives* angle_primitives = nullptr;

  ompl::base::StateSpacePtr state_space;
  TrigValues* trig_values_fast = nullptr;
...
public:
   ...
   */
  explicit NodeLattice(const uint32_t index);
    ...
    const unsigned int & goal_x, const unsigned int & goal_y)
  {
    ...
    NodeHybrid::resetObstacleHeuristic(costmap_master, start_x, start_y, goal_x, goal_y);
  }
   ...
   */
   void getNeighbors(
    ...
    NodeLattice * &)> & validity_checker,
    CollisionChecker * collision_checker,
    const bool & traverse_unknown,
    NodeVector & neighbors);


   inline void processPrimitiveInline(
