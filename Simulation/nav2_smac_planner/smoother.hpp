...
struct BoundaryPoints
{
   ...
   */
  BoundaryPoints(float & x_in, float & y_in, float & theta_in)
...
{
  float path_end_idx{0.0f};
  float expansion_path_length{0.0f};
  float original_path_length{0.0f};
  std::vector<BoundaryPoints> pts;
  bool in_collision{false};
};

typedef std::vector<BoundaryExpansion> BoundaryExpansions;
typedef std::vector<Pose>::iterator PathIterator;
typedef std::vector<Pose>::reverse_iterator ReversePathIterator;
...
protected:
  ...
  float min_turning_rad_, tolerance_, data_w_, smooth_w_;
