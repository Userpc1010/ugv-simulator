...
public:
   ...
   */
  bool isZoneValid(
    const NodePtr node, const float & radius, CollisionChecker * collision_checker,
  ...
  /**
   * @brief Супер-быстрая проверка расстояния в ячейках.
   * @param node Текущий узел.
   * @param dist_cells Порог в ячейках (например, 150).
   */
  inline bool isWithinDistanceCells(const NodePtr & node, const int dist_cells)
  {
    // Работаем напрямую с целочисленными координатами сетки
    // В Smac у pose обычно x и y — это индексы ячеек (float или int)
    const auto & goal = _goals_coordinate[0]; // Берем первую цель

    int dx = static_cast<int>(node->pose.x - goal.x);
    int dy = static_cast<int>(node->pose.y - goal.y);

    // Никаких std::hypot и float. Только умножение и сложение целых.
