...
public:
   ...
   */
  explicit Node2D(const uint32_t index);
  ...
  static inline Coordinates getCoords(
    const uint32_t & index, const unsigned int & width, const unsigned int & angles)
  {
    if (angles != 1) {
   ...
   * @return coordinates of point
   */
  static inline Coordinates getCoords(const uint32_t & index)
  {
