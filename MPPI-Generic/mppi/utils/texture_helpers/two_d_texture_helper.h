...
public:
  ...
  void updateTextureFromGPU(int layer, void* gpu_data, size_t count, bool column_major = false);
...
template <class DATA_T>
void TwoDTextureHelper<DATA_T>::updateTextureFromGPU(int layer, void* gpu_data, size_t count, bool column_major)
{
    // Проверяем границы
    if (layer >= this->size_) {
        throw std::runtime_error("updateTextureFromGPU: layer out of bounds");
    }

    TextureParams<DATA_T>* param = &this->textures_[layer];

    // Проверяем, что текстура выделена
    if (!param->allocated || param->array_d == nullptr) {
        throw std::runtime_error("updateTextureFromGPU: texture not allocated");
    }

    // Column-major не поддерживается для GPU (нужно транспонирование через ядро)
    if (column_major) {
        throw std::runtime_error("updateTextureFromGPU: column_major not supported for GPU data");
    }

    // Проверяем размер
    size_t expected_size = param->extent.width * param->extent.height;
    if (count != expected_size) {
        throw std::runtime_error("updateTextureFromGPU: size mismatch. Expected " +
                                 std::to_string(expected_size) + ", got " + std::to_string(count));
    }

    // Копируем данные напрямую Device-to-Device
    HANDLE_ERROR(cudaMemcpy2DToArrayAsync(
        param->array_d,           // dst array
        0, 0,                     // dst offset
        gpu_data,                 // src ptr (на GPU)
        param->extent.width * sizeof(DATA_T),  // src pitch
        param->extent.width * sizeof(DATA_T),  // width in bytes
