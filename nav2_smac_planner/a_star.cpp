...
template<typename NodeT>
void AStarAlgorithm<NodeT>::setCollisionChecker(CollisionChecker * collision_checker)
{
  ...
  const int & coarse_search_resolution)
{
    ...
    _goal_manager.hasGoalChanged(ref_goal_coord))
  {
    ...
    NodeT::resetObstacleHeuristic(_collision_checker->getCostmapMaster(), _start->pose.x, _start->pose.y, mx, my);
  }
  ...
  std::vector<std::tuple<float, float, float>> * expansions_log)
{
  ...
  addNode(0.0f, getStart());
  getStart()->setAccumulatedCost(0.0f);
  ...
  const uint32_t max_index = static_cast<uint32_t>(getSizeX()) *
    static_cast<uint32_t>(getSizeY()) *
    static_cast<uint32_t>(getSizeDim3());
  NodeGetter neighborGetter =
    [&, this](const uint32_t & index, NodePtr & neighbor_rtn) -> bool
    {
      if (index >= max_index) {
        return false;
      }
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    // 50 ячеек = 5 метров при разрешении 0.1м
    if (_goal_manager.isWithinDistanceCells(current_node, 50) && _costmap->getCost(current_node->getIndex()) == 0) {
    expansion_result = _expander->tryAnalyticExpansion(current_node, coarse_check_goals, fine_check_goals, _goal_manager.getGoalsCoordinates(), neighborGetter, analytic_iterations, closest_distance);
    }
