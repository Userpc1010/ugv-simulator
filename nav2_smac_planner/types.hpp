...
struct SmacPlannerLatticeDynamicParams {
  ...
  float analytic_expansion_max_length = 100.0f;    // максимальная длина аналитического расширения в метрах
  float analytic_expansion_max_cost = 200.0f;    // максимальная стоимость для аналитического расширения
  ...
  // Integer parameters
  int max_iterations = 2000000;                  // максимальное количество итераций поиска
  int max_on_approach_iterations = 2000;         // максимальное количество итераций при приближении к цели
  int terminal_checking_interval = 5000;         // интервал проверки условий завершения
...
struct SmacPlanner2DParams {
  ...
  float tolerance = 2.5f;                    // допуск достижения цели в метрах
  float cost_penalty = 1.0f;                   // множитель штрафа за стоимость клетки
  float max_planning_time = 5.0;              // максимальное время планирования в секундах
  ...
  bool allow_unknown = false;                   // разрешение планирования через неизвестные области
  int max_iterations = 2000000;                // максимальное количество итераций поиска
  int max_on_approach_iterations = 2000;       // макс. итераций при приближении к цели
  int terminal_checking_interval = 5000;       // интервал проверки условий завершения
...
struct CostmapConfig {
  ...
  float resolution = 0.489f;                    // Разрешение карты (метры на ячейку)
  unsigned int width = 800;                     // Ширина карты в ячейках
  unsigned int height = 800;                    // Высота карты в ячейках
