...
float Node2D::cost_travel_multiplier = 2.0f;

Node2D::Node2D(const uint32_t index)
: parent(nullptr),
...
{
  float normalized_cost = child->getCost() / 252.0f;
  ...
  if ((dx * dx + dy * dy) > 1.05f) {
    return sqrt_2 * (1.0f + cost_travel_multiplier * normalized_cost);
  }

  // Length = 1.0
  return 1.0f + cost_travel_multiplier * normalized_cost;
}
