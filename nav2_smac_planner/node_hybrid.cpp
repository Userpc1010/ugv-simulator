...
float NodeHybrid::size_lookup = 25.0f;
LookupTable NodeHybrid::dist_heuristic_lookup_table;
std::shared_ptr<Costmap2D_Master> NodeHybrid::costmap_master = nullptr;
  ...
  SearchInfo & search_info)
{
  ...
  float angle = 2.0f * asin(sqrt(2.0f) / (2 * min_turning_radius));
  ...
  bin_size = 2.0f * M_PIf32 / static_cast<float>(num_angle_quantization);
  float increments;
  if (angle < bin_size) {
    increments = 1.0f;
  } else {
  ...
  projections.emplace_back(delta_dist, 0.0f, 0.0f, TurnDirection::FORWARD);  // Forward
  ...
  for (unsigned int i = 0; i != projections.size(); i++) {
    ...
    for (unsigned int j = 0; j != num_angle_quantization; j++) {
      float cos_theta = cos(bin_size * j);
      float sin_theta = sin(bin_size * j);
      if (i == 0) {
  ...
  SearchInfo & search_info)
{
  ...
  projections.emplace_back(-delta_dist, 0.0f, 0.0f, TurnDirection::REVERSE);  // Backward
  projections.emplace_back(
...
unsigned int HybridMotionTable::getClosestAngularBin(const float & theta)
{
...
float HybridMotionTable::getAngle(const float & theta)
{
  return theta / bin_size;
}

NodeHybrid::NodeHybrid(const uint32_t index)
: parent(nullptr),
  pose(0.0f, 0.0f, 0.0f),
...
inline float distanceHeuristic2D(
  const uint32_t idx, const unsigned int size_x,
...
void NodeHybrid::resetObstacleHeuristic(
  std::shared_ptr<Costmap2D_Master> costmap_master_i,
  ...
  const unsigned int & goal_x, const unsigned int & goal_y)
{
  ...
  // than 0.05 * normalized cost. Since this is just a search prior, there's no loss in generality
    ...
    costmap_master = costmap_master_i;
    auto costmap = costmap_master->getCostmap();
  ...
  const SearchInfo & search_info)
{
  ...
  float angular_bin_size = 2 * M_PIf32 / static_cast<float>(dim_3_size);
...
void NodeHybrid::getNeighbors(
  std::function<bool(const uint32_t &,
  NodeHybrid * &)> & NeighborGetter,
  CollisionChecker * collision_checker,
  const bool & traverse_unknown,
