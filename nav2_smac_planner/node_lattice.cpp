...
float NodeLattice::size_lookup = 25.0f;
...
{
  cleanupFastAccessTables();
  cleanupMotionPrimitives();
  if (trig_values_fast) {
  delete[] trig_values_fast;
  }
}
  ...
  SearchInfo & search_info)
{
  ...
  // Сначала подсчитаем общее количество примитивов
  ...
  cleanupMotionPrimitives(); // Очищаем предыдущие данные
  motion_primitives_flat = new MotionPrimitive[total_primitives];
  motion_primitives_flat_size = 0; // Счетчик для заполнения массива

  // Теперь заполняем массив
  prev_start_angle = 0.0f;
  primitives.clear();
  ...
  for (unsigned int i = 0; i < json_primitives.size(); ++i) {
      ...
      if (i == 0 || prev_start_angle != new_primitive.start_angle) {
          // Сохраняем предыдущую группу
          if (!primitives.empty()) {
              motion_offsets.push_back(motion_primitives_flat_size);
              motion_counts.push_back(primitives.size());

              // Копируем примитивы из группы в массив
              for (const auto& prim : primitives) {
                  if (motion_primitives_flat_size < total_primitives) {
                      motion_primitives_flat[motion_primitives_flat_size] = prim;
                      motion_primitives_flat_size++;
                  }
              }
              primitives.clear();
          }
  ...
  createFastAccessTables();

  // Populate useful precomputed values to be leveraged
  if (trig_values_fast != nullptr) {
      delete[] trig_values_fast;
      trig_values_fast = nullptr;
  }

  trig_values_fast = new TrigValues [lattice_metadata.heading_angles.size()];

  for (unsigned int i = 0; i < lattice_metadata.heading_angles.size(); ++i) {
      trig_values_fast[i] = TrigValues(
...
void LatticeMotionTable::cleanupMotionPrimitives() {
    if (motion_primitives_flat != nullptr) {
        delete[] motion_primitives_flat;
        motion_primitives_flat = nullptr;
        motion_primitives_flat_size = 0;
    }
}

void LatticeMotionTable::cleanupFastAccessTables()
{
    if (angle_primitives) {
           delete[] angle_primitives;
           angle_primitives = nullptr;
       }
}

void LatticeMotionTable::createFastAccessTables()
{
    // Освобождаем старые данные
        cleanupFastAccessTables();

        // Выделяем память для быстрого доступа
        angle_primitives = new AnglePrimitives[num_angle_quantization];

        // Заполняем таблицу быстрого доступа
        for (unsigned int angle = 0; angle < num_angle_quantization; ++angle) {
            AnglePrimitives& ap = angle_primitives[angle];

            // Прямые примитивы
            if (angle < motion_offsets.size()) {
                size_t start = motion_offsets[angle];
                ap.forward_primitives = &motion_primitives_flat[start];
                ap.forward_count = motion_counts[angle];
            } else {
                ap.forward_primitives = nullptr;
                ap.forward_count = 0;
            }

            // Вычисляем обратный угол
            unsigned int reverse_angle = angle - (num_angle_quantization / 2);
            if (reverse_angle >= num_angle_quantization) {
                reverse_angle -= num_angle_quantization;
            } else if (static_cast<int>(reverse_angle) < 0) {
                reverse_angle += num_angle_quantization;
            }

            // Обратные примитивы
            if (reverse_angle < motion_offsets.size()) {
                size_t start = motion_offsets[reverse_angle];
                ap.reverse_primitives = &motion_primitives_flat[start];
                ap.reverse_count = motion_counts[reverse_angle];
            } else {
                ap.reverse_primitives = nullptr;
                ap.reverse_count = 0;
            }
        }
}

LatticeMotionTable::PrimitiveBuffers LatticeMotionTable::getPrimitiveBuffers(const NodeLattice* node) const
{
    unsigned int angle_idx = node->pose.theta;

        if (angle_idx >= num_angle_quantization || !angle_primitives) {
            PrimitiveBuffers buffers;
            buffers.forward = nullptr;
            buffers.forward_count = 0;
            buffers.reverse = nullptr;
            buffers.reverse_count = 0;
            return buffers;
        }

        const AnglePrimitives& ap = angle_primitives[angle_idx];

        PrimitiveBuffers buffers;
        buffers.forward = ap.forward_primitives;
        buffers.forward_count = ap.forward_count;
        buffers.reverse = allow_reverse_expansion ? ap.reverse_primitives : nullptr;
        buffers.reverse_count = allow_reverse_expansion ? ap.reverse_count : 0;
...
unsigned int LatticeMotionTable::getClosestAngularBin(const float & theta)
{
...
float LatticeMotionTable::getAngle(const float & theta)
{
    return getClosestAngularBin(theta);
}

NodeLattice::NodeLattice(const uint32_t index)
: parent(nullptr),
  pose(0.0f, 0.0f, 0.0f),
  ...
  bool is_backwards)
{
  ...
  static const float bin_size = 2.0f * M_PIf32 / collision_checker->getPrecomputedAngles().size();
  const float angle = std::fmod(motion_table.getAngleFromBin(this->pose.theta), 2.0f * M_PIf32) / bin_size;
  ...
  if (motion_primitive) {
    ...
    MotionPose last_pose(1e7f, 1e7f, 1e7f, TurnDirection::UNKNOWN);
    ...
    for (auto it = motion_primitive->poses.begin(); it != motion_primitive->poses.end(); ++it) {
      ...
      if (pose_dist._x * pose_dist._x + pose_dist._y * pose_dist._y > resolution_diag_sq) {
        ...
        if (is_backwards) {
          prim_pose._theta = std::fmod(it->_theta + M_PIf32, 2.0f * M_PIf32);
        } else {
          prim_pose._theta = std::fmod(it->_theta, 2.0f * M_PIf32);
        }
        if (collision_checker->inCollision(
            prim_pose._x,
            prim_pose._y,
...
float NodeLattice::getTraversalCost(const NodePtr & child)
{
  ...
  if (transition_prim->trajectory_length < 1e-4f) {
    return motion_table.rotation_penalty * (1.0f + motion_table.cost_penalty * normalized_cost);
  }
  ...
  const float & obstacle_heuristic)
{
  ...
  const TrigValues & trig_vals = motion_table.trig_values_fast[static_cast<unsigned int>(goal_coords.theta)];
  ...
  float dtheta_bin = node_coords.theta - goal_coords.theta;
  if (dtheta_bin < 0) {
  ...
  float motion_heuristic = 0.0f;
  const int floored_size = floor(size_lookup / 2.0f);
  const int ceiling_size = ceil(size_lookup / 2.0f);
  ...
  if (abs(node_coords_relative.x) < floored_size && mirrored_relative_y < floored_size) {
    ...
    int theta_pos;
    if (node_coords_relative.y < 0.0f) {
  ...
  } else if (obstacle_heuristic == 0.0f) {
  ...
  const SearchInfo & search_info)
{
  ...
  dist_heuristic_lookup_table.resize(size_lookup * ceil(size_lookup / 2.0f) * dim_3_size_int);
  for (float x = ceil(-size_lookup / 2.0f); x <= floor(size_lookup / 2.0f); x += 1.0f) {
    for (float y = 0.0f; y <= floor(size_lookup / 2.0f); y += 1.0f) {
...
void NodeLattice::getNeighbors(
    std::function<bool(const uint32_t &, NodeLattice * &)> & NeighborGetter,
    CollisionChecker * collision_checker,
    const bool & traverse_unknown,
    NodeVector & neighbors)
{
    // Получаем структуру с указателями
        auto buffers = motion_table.getPrimitiveBuffers(this);
        ...
        const float & grid_resolution = motion_table.lattice_metadata.grid_resolution;
        const unsigned int half_quantization = motion_table.num_angle_quantization / 2;

        // Обработка прямых примитивов
        for (size_t i = 0; i < buffers.forward_count; ++i) {
            // Доступ через смещение в одномерном массиве
            MotionPrimitive* prim = buffers.forward + i;

            // Вычисляем проекцию
            Coordinates proj;
            const MotionPose & end_pose = prim->poses.back();

            proj.x = this->pose.x + (end_pose._x / grid_resolution);
            proj.y = this->pose.y + (end_pose._y / grid_resolution);
            proj.theta = prim->end_angle; // Прямой угол

            processPrimitiveInline(
                prim, proj, false,
                NeighborGetter, collision_checker,
                traverse_unknown, neighbors
            );
        }

        // Обработка обратных примитивов
        if (buffers.reverse && buffers.reverse_count > 0) {
            for (size_t i = 0; i < buffers.reverse_count; ++i) {
                // Доступ через смещение в одномерном массиве
                MotionPrimitive* prim = buffers.reverse + i;

                // Вычисляем проекцию с корректировкой угла
                Coordinates proj;
                const MotionPose & end_pose = prim->poses.back();
                ...
                // Коррекция угла для обратного движения
                proj.theta = prim->end_angle - half_quantization;
                if (proj.theta >= motion_table.num_angle_quantization) {
                    proj.theta -= motion_table.num_angle_quantization;
                } else if (static_cast<int>(proj.theta) < 0) {
                    proj.theta += motion_table.num_angle_quantization;
                }
...
inline void NodeLattice::processPrimitiveInline(
...
{
    // Получаем индекс
    uint32_t index = NodeLattice::getIndex(
        static_cast<unsigned int>(projection.x),
        static_cast<unsigned int>(projection.y),
        static_cast<unsigned int>(projection.theta));

    NodePtr neighbor = nullptr;
    if (NeighborGetter(index, neighbor) && !neighbor->wasVisited()) {
        Coordinates initial_pose = neighbor->pose;
        neighbor->setPose(projection);

        if (neighbor->isNodeValid(traverse_unknown, collision_checker, primitive, is_reverse)) {
            neighbor->setMotionPrimitive(primitive);
            neighbor->backwards(is_reverse);
            neighbors.push_back(neighbor);
        } else {
            neighbor->setPose(initial_pose);
        }
    }
}
  ...
  if (prim) {
    ...
    for (auto it = prim->poses.crbegin(); it != prim->poses.crend(); ++it) {
      ...
      if (current_node->isBackward()) {
        prim_pose.theta = std::fmod(it->_theta + M_PIf32, 2.0f * M_PIf32);
      } else {
